...
class FolioAuthorizationDevelopmentTest extends FinancialDevelopmentFixture {
    ...
    @Test void hrAndCustomerCannotProcessRefundRegardlessOfGuessedId()throws Exception{long b=stay("CUSTOMER");String body="{\"requestKey\":\"process_x1\",\"reason\":\"review\"}";for(String role:List.of("CUSTOMER","HR_ADMIN"))mvc.perform(post("/api/folios/{folio}/refunds/{refund}/confirm",folio(b),Long.MAX_VALUE).with(authentication(auth(role))).contentType("application/json").content(body)).andExpect(status().isForbidden());}
    @ParameterizedTest @ValueSource(strings={"STAFF","MANAGER","OWNER","SUPER_ADMIN"}) void operatingRolesCanReadCustomerFolio(String role)throws Exception{long b=stay("CUSTOMER");mvc.perform(get("/api/admin/billing/bookings/{id}/folio",b).with(authentication(auth(role)))).andExpect(status().isOk()).andExpect(jsonPath("$.data.bookingId").value(b));mvc.perform(get("/api/admin/billing/folios/{id}",folio(b)).with(authentication(auth(role)))).andExpect(status().isOk());}
    @Test void staffCanRecordExistingBusinessPaymentButCannotApproveRefund()throws Exception{long b=stay("CUSTOMER");mvc.perform(post("/api/admin/billing/folios/{id}/payments",folio(b)).with(authentication(auth("STAFF"))).contentType("application/json").content(paymentBody())).andExpect(status().isOk());String body="{\"requestKey\":\"process_x1\",\"reason\":\"review\"}";mvc.perform(post("/api/folios/{folio}/refunds/{refund}/confirm",folio(b),Long.MAX_VALUE).with(authentication(auth("STAFF"))).contentType("application/json").content(body)).andExpect(status().isForbidden());}
    @Test void paymentServiceRejectsHrCustomerAndInactiveOperator(){long b=stay("CUSTOMER");for(String role:List.of("HR_ADMIN","CUSTOMER"))assertThrows(AccessDeniedException.class,()->payments.recordPayment(folio(b),payRequest("10"),uid(role)));jdbc.update("UPDATE sys_user SET status=0 WHERE id=?",uid("STAFF"));assertThrows(AccessDeniedException.class,()->payments.recordPayment(folio(b),payRequest("10"),uid("STAFF")));assertEquals(0,jdbc.queryForObject("SELECT COUNT(*) FROM payment WHERE folio_id=?",Integer.class,folio(b)));}
    @Test void inactiveCustomerTokenCannotReadOwnFolio()throws Exception{long b=stay("CUSTOMER");jdbc.update("UPDATE sys_user SET status=0 WHERE id=?",uid("CUSTOMER"));mvc.perform(get("/api/bookings/{id}/folio",b).with(customer(uid("CUSTOMER")))).andExpect(status().isForbidden());}
    @Test void inactiveCustomerTokenCannotReadRefunds()throws Exception{long b=stay("CUSTOMER");jdbc.update("UPDATE sys_user SET status=0 WHERE id=?",uid("CUSTOMER"));mvc.perform(get("/api/folios/{id}/refunds",folio(b)).with(customer(uid("CUSTOMER")))).andExpect(status().isForbidden());}
    @Test void unauthenticatedBillingEndpointsReturn401()throws Exception{long b=stay("CUSTOMER");mvc.perform(get("/api/bookings/{id}/folio",b).with(anonymous())).andExpect(status().isUnauthorized());mvc.perform(get("/api/admin/billing/folios/{id}",folio(b)).with(anonymous())).andExpect(status().isUnauthorized());mvc.perform(get("/api/folios/{id}/refunds",folio(b)).with(anonymous())).andExpect(status().isUnauthorized());}
}
