...
class RefundSettlementDevelopmentTest extends FinancialDevelopmentFixture {
    ...
    @ParameterizedTest @ValueSource(booleans={true,false}) void fullWalletCheckoutAndBlockedSemantics(boolean blocked){long b=stay();creditWallet("350");if(blocked){as("STAFF");service.status(wid("CUSTOMER"),state(WalletStatus.BLOCKED));}clock.day(3);checkout(b);assertEquals(2,stayState(b));assertEquals(new BigDecimal("50.00"),wallets.find(wid("CUSTOMER")).getBalance());var view=queries.byBooking(b,uid("CUSTOMER"));assertEquals(1,view.payments().size());assertEquals("WALLET",view.payments().get(0).getPaymentMethod());var rows=wallets.transactions(wid("CUSTOMER"),0);assertEquals(2,rows.size());assertEquals(new BigDecimal("-300.00"),rows.get(1).getAmount());assertEquals(3,rows.get(1).getTransactionType());assertThrows(BusinessException.class,()->checkout(b));invariantBooking(b);}
    ...
    @Test void differentFoliosSharingWalletCannotOverdraw(){long a=stay();long b=createBooking("CUSTOMER");var r=new ApproveBookingRequest();r.setAssignedRoomId(room2);bookings.approveBooking(b,r,uid("MANAGER"));register(b);bookings.checkIn(b,uid("MANAGER"));creditWallet("300");clock.day(3);
