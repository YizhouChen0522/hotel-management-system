...
class RefundSettlementDevelopmentTest extends FinancialDevelopmentFixture {
    @Override BigDecimal reservationFundingAmount(){return new BigDecimal("300.00");}
    @Override void afterCheckIn(long booking){as("MANAGER");var charge=expenses.register(folio(booking),expense("300"));expenses.confirm(folio(booking),charge.id());}
    long nonDepositPaymentCount(long booking){return queries.byBooking(booking,uid("CUSTOMER")).payments().stream().filter(p->!"DEPOSIT_TRANSFER".equals(p.getPaymentMethod())).count();}
    java.util.List<WalletTransaction> walletTransactions(String source){return wallets.transactions(wid("CUSTOMER"),0).stream().filter(t->source.equals(t.getSourceType())).toList();}
    ...
    @Test void confirmRetryAndBlockedWalletCreditAndSummaryJson()throws Exception{
        ...
        var ledger=walletTransactions("REFUND");assertEquals(1,ledger.size());assertEquals(WalletTransactionType.REFUND_CREDIT.getCode(),ledger.get(0).getTransactionType());assertEquals(r.id(),ledger.get(0).getSourceId());assertEquals("REFUND",ledger.get(0).getSourceType());
    ...
    void refundFailureIsAtomic(String statement){long b=stay();pay(b,"400");var r=requestRefund(b,"100","refund_01");as("MANAGER");gate.arm(Thread.currentThread().getName(),statement,true);assertThrows(Exception.class,()->refunds.confirm(folio(b),r.id(),process("process_01")));assertEquals(RefundStatus.PENDING,refunds.list(folio(b)).get(0).status());assertEquals(0,wallets.find(wid("CUSTOMER")).getBalance().signum());assertEquals(0,walletTransactions("REFUND").size());invariantBooking(b);}
    @ParameterizedTest @ValueSource(booleans={true,false}) void fullWalletCheckoutAndBlockedSemantics(boolean blocked){long b=stay();creditWallet("350");if(blocked){as("STAFF");service.status(wid("CUSTOMER"),state(WalletStatus.BLOCKED));}clock.day(3);checkout(b);assertEquals(2,stayState(b));assertEquals(new BigDecimal("50.00"),wallets.find(wid("CUSTOMER")).getBalance());var view=queries.byBooking(b,uid("CUSTOMER"));var payment=view.payments().stream().filter(p->"WALLET".equals(p.getPaymentMethod())).toList();assertEquals(1,payment.size());var rows=walletTransactions("PAYMENT");assertEquals(1,rows.size());assertEquals(new BigDecimal("-300.00"),rows.get(0).getAmount());assertEquals(3,rows.get(0).getTransactionType());assertThrows(BusinessException.class,()->checkout(b));invariantBooking(b);}
    @Test void partialContributionCommitsOnceAndOrdinaryPaymentFinishesCheckout()throws Exception{
        long b=stay();creditWallet("100");clock.day(3);assertThrows(WalletSettlementIncompleteException.class,()->checkout(b));assertEquals(1,stayState(b));assertEquals(new BigDecimal("200.00"),queries.byBooking(b,uid("CUSTOMER")).balanceAmount());assertEquals(0,wallets.find(wid("CUSTOMER")).getBalance().signum());
        creditWallet("50");mvc.perform(post("/api/admin/bookings/"+b+"/check-out").with(authentication(auth("MANAGER")))).andExpect(status().isConflict());assertEquals(new BigDecimal("50.00"),wallets.find(wid("CUSTOMER")).getBalance());assertEquals(1,nonDepositPaymentCount(b));pay(b,"200");checkout(b);invariantBooking(b);
    }
    @Test void zeroWalletLeavesOutstandingDebtAndNegativeBalanceRequiresExplicitRefund(){long b=stay();clock.day(3);assertThrows(BusinessException.class,()->checkout(b));assertEquals(0,nonDepositPaymentCount(b));pay(b,"400");assertThrows(BusinessException.class,()->checkout(b));assertEquals(0,refundsCount(b));invariantBooking(b);}
    int refundsCount(long b){return jdbc.queryForObject("SELECT COUNT(*) FROM refund WHERE folio_id=?",Integer.class,folio(b));}
    @ParameterizedTest @ValueSource(strings={"PaymentMapper.insert","WalletPostingMapper.append","WalletPostingMapper.debit","SysAuditLogMapper.insert","FolioMapper.close","StayMapper.close","RoomMapper.transitionStatus"})
    void checkoutFinancialAndLifecycleFailuresRollbackTogether(String statement){long b=stay();creditWallet("300");clock.day(3);gate.arm(Thread.currentThread().getName(),statement,true);assertThrows(Exception.class,()->checkout(b));assertEquals(1,stayState(b));assertEquals(new BigDecimal("300.00"),wallets.find(wid("CUSTOMER")).getBalance());assertEquals(0,nonDepositPaymentCount(b));assertNull(queries.byBooking(b,uid("CUSTOMER")).closedTime());invariantBooking(b);}
    ...
    @Test void reusedProcessingKeyRollsBackSecondCredit(){
        ...
        assertEquals(new BigDecimal("50.00"),wallets.find(wid("CUSTOMER")).getBalance());assertEquals(1,walletTransactions("REFUND").size());invariantBooking(b);
    }
    @Test void ordinaryPaymentCannotForgeWalletSource(){long b=stay();var r=payRequest("100");r.setPaymentMethod("WALLET");assertThrows(BusinessException.class,()->payments.recordPayment(folio(b),r,uid("STAFF")));assertEquals(0,nonDepositPaymentCount(b));invariantBooking(b);}
    ...
    @Test void concurrentConfirmCannotDoubleCredit(){long b=stay();pay(b,"400");var r=requestRefund(b,"100","refund_01");assertNull(serialized("RefundMapper.process",()->refunds.confirm(folio(b),r.id(),process("process_01")),()->refunds.confirm(folio(b),r.id(),process("process_01"))));assertEquals(1,walletTransactions("REFUND").size());invariantBooking(b);}
    @Test void concurrentCheckoutsCannotDoubleDebit(){long b=stay();creditWallet("300");clock.day(3);assertInstanceOf(BusinessException.class,serialized("PaymentMapper.insert",()->checkout(b),()->checkout(b)));assertEquals(1,nonDepositPaymentCount(b));invariantBooking(b);}
    @Test void differentFoliosSharingWalletCannotOverdraw(){long a=stay();long b=createBooking("CUSTOMER");var r=new ApproveBookingRequest();r.setAssignedRoomId(room2);bookings.approveBooking(b,r,uid("MANAGER"));register(b);bookings.checkIn(b,uid("MANAGER"));afterCheckIn(b);creditWallet("300");clock.day(3);
    ...
    @ParameterizedTest @ValueSource(booleans={true,false}) void existingPaymentSameAndDifferentKeyConcurrency(boolean same){long b=stay();var a=payRequest("10");var c=same?a:payRequest("15");assertNull(serialized("FolioMapper.selectByIdForUpdate",()->payments.recordPayment(folio(b),a,uid("STAFF")),()->payments.recordPayment(folio(b),c,uid("STAFF"))));assertEquals(same?1:2,nonDepositPaymentCount(b));invariantBooking(b);}
    @Test void existingPaymentWinsCheckoutSeesCredit(){long b=stay();pay(b,"300");clock.day(3);assertInstanceOf(BusinessException.class,serialized("PaymentMapper.insert",()->pay(b,"1"),()->checkout(b)));assertEquals(1,stayState(b));invariantBooking(b);}
    ...
    @Test void existingMissingNightOrRoomChargeCannotBeHiddenByWallet(){long b=stay();creditWallet("300");jdbc.update("DELETE FROM folio_item WHERE folio_id=? ORDER BY id LIMIT 1",folio(b));financial.recalculateSummary(folio(b));clock.day(3);assertThrows(BusinessException.class,()->checkout(b));assertEquals(new BigDecimal("300.00"),wallets.find(wid("CUSTOMER")).getBalance());invariantBooking(b);}
    @ParameterizedTest @ValueSource(strings={"room","assignment","event","reversal"}) void existingCheckoutCorruptHistoryStillBlocks(String kind){long b=stay();clock.day(1);var change=new ChangeRoomDuringStayRequest();change.setNewRoomId(room3);change.setReason("test move");bookings.changeRoomDuringStay(b,change,uid("MANAGER"));creditWallet("500");clock.day(3);
        switch(kind){case "room"->jdbc.update("UPDATE room SET status=1 WHERE id=?",room3);case "assignment"->jdbc.update("UPDATE stay_room_assignment SET room_id=? WHERE stay_id=? AND end_time IS NULL",room4,sid(b));case "event"->jdbc.update("DELETE FROM room_billing_event WHERE stay_id=(SELECT id FROM stay WHERE booking_id=?)",b);default->jdbc.update("DELETE FROM folio_item WHERE folio_id=? AND item_type='ROOM_RATE_ADJUSTMENT' ORDER BY id LIMIT 1",folio(b));}
