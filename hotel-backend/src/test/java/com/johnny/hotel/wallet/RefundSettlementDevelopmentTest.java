...
class RefundSettlementDevelopmentTest extends FinancialDevelopmentFixture {
    ...
    @Test void idorAndCrossFolioRefundReferencesAreDenied()throws Exception{long b=stay();pay(b,"400");var r=requestRefund(b,"100","refund_01");long other=createBooking("OTHER_CUSTOMER");
        mvc.perform(get("/api/folios/"+folio(b)+"/refunds").with(authentication(auth("OTHER_CUSTOMER")))).andExpect(status().isForbidden());as("OTHER_CUSTOMER");assertThrows(Exception.class,()->refunds.create(folio(b),refundRequest("1","refund_02")));as("MANAGER");assertThrows(Exception.class,()->refunds.confirm(folio(other),r.id(),process("process_01")));invariantBooking(b);}
