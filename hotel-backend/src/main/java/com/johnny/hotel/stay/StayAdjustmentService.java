...
public class StayAdjustmentService {
    ...
    private final StayContext stayContext;
    ...
    private final com.johnny.hotel.guest.GuestAccess operationalAccess;
    private final SysAuditLogMapper audits;
    private Long actor(){Long id=access.actor();access.operational(id,false);return id;}
    @Transactional
    public List<StayAdjustment> list(Long stayId){actor();require(stayContext.lockReservation(stayId)!=null,"Booking does not exist");return adjustments.forStay(stayId);}
    @Transactional
    public StayAdjustment apply(Long stayId,StayAdjustmentRequest request) {
        Long actor=actor();operationalAccess.employee(actor);require(request!=null&&request.getType()!=null,"Adjustment type is required");
        ...
        var booking=stayContext.lockReservation(stayId);require(booking!=null,"Booking does not exist");
        var history=adjustments.forStay(stayId);
        ...
        var stay=stays.lock(stayId);
        require(stay!=null && stay.getStatus()==StayStatus.IN_HOUSE.getCode(),"Stay adjustments require an in-house Stay");
        var assignment=conflicts.activeAssignment(stay.getId());
        require(assignment!=null,"Active assignment is missing");
        var room=rooms.selectByIdForUpdate(assignment.getRoomId());
        assignment=assignments.selectActiveByStayId(stayId);
        ...
        var folio=folios.selectByStayIdForUpdate(stayId);require(folio!=null&&folio.getClosedTime()==null,"Folio is finalized");
        ...
        var a=StayAdjustment.builder().stayId(stayId).folioId(folio.getId()).assignmentId(assignment.getId()).operatorId(actor)
        ...
        if(request.getType()==StayAdjustmentType.EXTENSION) {
            ...
            var collisions=conflicts.overlapping(room.getId(),booking.getId(),end,request.getNewEnd());
            ...
            for(var n:quote.getNightlyRates()) {
                one(adjustments.insertNight(ExtensionNightlyRate.builder().adjustmentId(a.getId()).stayId(stayId).roomTypeId(room.getRoomTypeId()).stayDate(n.getStayDate()).rateAmount(n.getPrice()).rateSource(n.getRateSource()).currency(folio.getCurrency()).build()));
            ...
            var arrivals=conflicts.arriving(room.getId(),booking.getId(),today);
        ...
        if(!commands.isEmpty())ledger.addItems(folio.getStayId(),commands,actor);
        one(audits.insert(SysAuditLog.builder().operatorId(actor).targetUserId(booking.getUserId()).action("STAY_"+request.getType().name())
                .detail("Booking "+stayId+", adjustment "+a.getId()+", effective end "+a.getOldEnd()+" -> "+a.getNewEnd()+", reason: "+a.getReason()+(a.getConflictBookingId()==null?"":"; conflicting booking "+a.getConflictBookingId()+" requires manual relocation / Room Change")).build()));
        return adjustments.forStay(stayId).stream().filter(r->r.getId().equals(a.getId())).findFirst().orElseThrow();
    }
