...
@Mapper public interface TaskAssignmentMapper {
 ...
 @Select("SELECT COUNT(*) FROM task_assignment WHERE task_id=#{task} AND is_current=1 FOR UPDATE") int currentCount(Long task);
 @Select("SELECT COUNT(*) FROM task_assignment WHERE task_id=#{task} AND is_current=1 AND status != 2") int incompleteCount(Long task);
 @Select("SELECT COALESCE(MAX(assignment_round),0) FROM task_assignment WHERE task_id=#{task} FOR UPDATE") int maxRound(Long task);
