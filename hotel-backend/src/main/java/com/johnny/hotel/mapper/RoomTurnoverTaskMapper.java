...
public interface RoomTurnoverTaskMapper {
    @Select("SELECT * FROM stay_room_assignment WHERE id=#{id}")
    StayRoomAssignment assignmentIdentity(Long id);
    @Select("SELECT t.id,t.task_id,t.room_id,t.stay_id,t.assignment_id,h.status,(SELECT a.assignee_user_id FROM task_assignment a WHERE a.task_id=t.task_id AND a.is_current=1 ORDER BY a.id LIMIT 1) accepted_by,(SELECT a.accepted_time FROM task_assignment a WHERE a.task_id=t.task_id AND a.is_current=1 ORDER BY a.id LIMIT 1) accepted_time,h.completed_time,(SELECT r.actor_user_id FROM task_record r WHERE r.task_id=t.task_id AND r.record_type IN (4,5) ORDER BY r.id DESC LIMIT 1) completed_by,COALESCE((SELECT r.detail FROM task_record r WHERE r.task_id=t.task_id AND r.record_type IN (4,5) ORDER BY r.id DESC LIMIT 1),t.note) note,t.create_time,t.update_time FROM room_turnover_task t JOIN hotel_task h ON h.id=t.task_id WHERE t.assignment_id=#{id} FOR UPDATE")
    ...
    @Select("SELECT t.id,t.task_id,t.room_id,t.stay_id,t.assignment_id,h.status,t.note,t.create_time,t.update_time FROM room_turnover_task t JOIN hotel_task h ON h.id=t.task_id WHERE t.id=#{id} FOR UPDATE")
    ...
    @Select("""
        <script>SELECT t.id,t.task_id,t.room_id,t.stay_id,t.assignment_id,h.status,t.note,t.create_time,t.update_time FROM room_turnover_task t JOIN hotel_task h ON h.id=t.task_id
