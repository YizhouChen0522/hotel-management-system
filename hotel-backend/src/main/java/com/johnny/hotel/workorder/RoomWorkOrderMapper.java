...
@Mapper public interface RoomWorkOrderMapper {
 ...
 @Select("<script>SELECT COUNT(*) FROM room_work_order <where><if test='status!=null'>status=#{status}</if><if test='room!=null'> AND room_id=#{room}</if></where></script>") long count(@Param("status")Integer status,@Param("room")Long room);
