...
@Mapper public interface InspectionMapper{
 ...
 @Insert("INSERT INTO housekeeping_inspection(room_id,booking_id,assignment_id,turnover_task_id,repair_order_id,status) VALUES(#{roomId},#{bookingId},#{assignmentId},#{turnoverTaskId},#{repairOrderId},0)")@Options(useGeneratedKeys=true,keyProperty="id")int insertForRepair(RoomInspection i);
 @Select("SELECT * FROM housekeeping_inspection WHERE repair_order_id=#{id} FOR UPDATE")RoomInspection byRepair(Long id);
 @Select("<script>SELECT * FROM housekeeping_inspection <where><if test='room!=null'>room_id=#{room}</if><if test='booking!=null'> AND booking_id=#{booking}</if><if test='employee!=null'> AND inspector_id=#{employee}</if><if test='status!=null'> AND status=#{status}</if></where> ORDER BY COALESCE(inspected_time,create_time) DESC,id DESC LIMIT #{offset},#{size}</script>")List<RoomInspection> page(@Param("room")Long room,@Param("booking")Long booking,@Param("employee")Long employee,@Param("status")Integer status,@Param("offset")int offset,@Param("size")int size);
 @Select("<script>SELECT COUNT(*) FROM housekeeping_inspection <where><if test='room!=null'>room_id=#{room}</if><if test='booking!=null'> AND booking_id=#{booking}</if><if test='employee!=null'> AND inspector_id=#{employee}</if><if test='status!=null'> AND status=#{status}</if></where></script>")long count(@Param("room")Long room,@Param("booking")Long booking,@Param("employee")Long employee,@Param("status")Integer status);
}
