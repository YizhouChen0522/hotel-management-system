...
public interface DamageAssessmentMapper {
 @Insert("INSERT INTO damage_assessment(room_id,booking_id,assignment_id,folio_id,source_type,source_id,damage_description,request_key,reported_by) VALUES(#{roomId},#{bookingId},#{assignmentId},#{folioId},#{sourceType},#{sourceId},#{damageDescription},#{requestKey},#{reportedBy})")
 ...
 @Select("SELECT r.room_id,i.booking_id,i.assignment_id FROM room_repair_task r JOIN housekeeping_inspection i ON i.id=r.inspection_id WHERE r.id=#{id}") DamageSourceFacts repairFacts(Long id);
 @Select("<script>SELECT * FROM damage_assessment <where><if test='booking!=null'>booking_id=#{booking}</if><if test='room!=null'> AND room_id=#{room}</if><if test='status!=null'> AND status=#{status}</if><if test='responsibility!=null'> AND responsibility=#{responsibility}</if><if test='sourceType!=null'> AND source_type=#{sourceType}</if></where> ORDER BY create_time DESC,id DESC LIMIT #{offset},#{size}</script>")
 ...
 @Select("<script>SELECT COUNT(*) FROM damage_assessment <where><if test='booking!=null'>booking_id=#{booking}</if><if test='room!=null'> AND room_id=#{room}</if><if test='status!=null'> AND status=#{status}</if><if test='responsibility!=null'> AND responsibility=#{responsibility}</if><if test='sourceType!=null'> AND source_type=#{sourceType}</if></where></script>")
 ...
 record DamageSourceFacts(Long roomId,Long bookingId,Long assignmentId){}
}
