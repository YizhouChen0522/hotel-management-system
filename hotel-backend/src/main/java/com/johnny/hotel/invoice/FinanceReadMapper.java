...
@Mapper public interface FinanceReadMapper{
 ...
 @Select("SELECT * FROM booking_room_assignment ORDER BY id DESC LIMIT #{offset},#{size}")List<Map<String,Object>> assignments(@Param("offset")int offset,@Param("size")int size);
 ...
 @Select("SELECT * FROM stay_history ORDER BY id DESC LIMIT #{offset},#{size}")List<Map<String,Object>> stays(@Param("offset")int offset,@Param("size")int size);
 @Select("SELECT id,linked_user_id,first_name,last_name,phone,email,gender,date_of_birth,nationality,document_type,CASE WHEN document_number IS NULL THEN NULL ELSE CONCAT('****',RIGHT(document_number,4)) END document_number,issuing_country,document_expiry_date,status,create_time,update_time FROM guest_profile ORDER BY id DESC LIMIT #{offset},#{size}")List<Map<String,Object>> guests(@Param("offset")int offset,@Param("size")int size);
