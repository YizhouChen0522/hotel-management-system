...
@Mapper public interface GuestMapper {
 ...
 @Select("""
 SELECT s.id AS stayId,s.booking_id AS bookingId,a.id AS assignmentId,COALESCE(h.room_number,r.room_number) AS roomNumber,COALESCE(h.room_type_name,rt.type_name) AS roomTypeName,a.start_time AS actualCheckInTime,a.end_time AS actualCheckOutTime FROM stay_guest bg JOIN stay s ON s.id=bg.stay_id JOIN stay_room_assignment a ON a.stay_id=s.id JOIN room r ON r.id=a.room_id JOIN room_type rt ON rt.id=a.room_type_id LEFT JOIN stay_history h ON h.assignment_id=a.id WHERE bg.guest_id=#{guestId} AND a.end_time IS NOT NULL ORDER BY a.start_time DESC
 """) List<GuestViews.Stay> stays(Long guestId);
 @Select("SELECT s.id AS stayId,s.booking_id AS bookingId,a.id AS assignmentId,COALESCE(h.room_number,r.room_number) AS roomNumber,COALESCE(h.room_type_name,rt.type_name) AS roomTypeName,a.start_time AS actualCheckInTime,a.end_time AS actualCheckOutTime FROM stay_guest bg JOIN stay s ON s.id=bg.stay_id JOIN stay_room_assignment a ON a.stay_id=s.id JOIN room r ON r.id=a.room_id JOIN room_type rt ON rt.id=a.room_type_id LEFT JOIN stay_history h ON h.assignment_id=a.id WHERE bg.guest_id=#{guestId} AND a.end_time IS NOT NULL ORDER BY a.start_time DESC,a.id DESC LIMIT #{offset},#{size}")List<GuestViews.Stay> stayPage(@Param("guestId")Long guestId,@Param("offset")int offset,@Param("size")int size);
 @Select("SELECT COUNT(*) FROM stay_guest bg JOIN stay s ON s.id=bg.stay_id JOIN stay_room_assignment a ON a.stay_id=s.id WHERE bg.guest_id=#{guestId} AND a.end_time IS NOT NULL")long stayCount(Long guestId);
}
