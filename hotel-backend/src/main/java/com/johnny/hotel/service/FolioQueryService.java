...
public class FolioQueryService {
    ...
    private FolioVO view(Folio f, Long customerId) {
        ...
        return FolioVO.builder().id(f.getId()).stayId(f.getStayId()).bookingId(f.getBookingId()).currency(f.getCurrency()).status(f.getStatus())
                .totalAmount(f.getTotalAmount()).paidAmount(f.getPaidAmount()).refundedAmount(f.getRefundedAmount()).arTransferredAmount(f.getArTransferredAmount()).balanceAmount(f.getBalanceAmount()).closedTime(f.getClosedTime())
