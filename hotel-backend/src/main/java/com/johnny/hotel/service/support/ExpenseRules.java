...
public final class ExpenseRules {
    ...
    public static void validate(RegisterExpenseRequest r, Booking b, LocalDate today,LocalDate effectiveEnd) {
    ...
    public static List<FolioItem> checkout(Booking booking,Long folioId,List<ExpenseRegistration> all,List<FolioItem> ledger) {return checkout(booking,folioId,all,ledger,booking.getCheckOutDate());}
    public static List<FolioItem> checkout(Booking booking,Long folioId,List<ExpenseRegistration> all,List<FolioItem> ledger,LocalDate effectiveEnd) {
        ...
        for(var e:all) {
            ...
            validate(request,booking,effectiveEnd,effectiveEnd);
        ...
        require(remainder.stream().allMatch(i->Set.of("ROOM_CHARGE","ROOM_RATE_ADJUSTMENT","EARLY_CHECKOUT_REVERSAL").contains(i.getItemType())),"Unregistered or unmatched extra fee ledger");
        return remainder;
    }
}
